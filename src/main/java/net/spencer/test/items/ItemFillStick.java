...
public class ItemFillStick extends Item {

	public BlockPos[] posA = { new BlockPos(0, 0, 0), new BlockPos(0, 0, 0) };
	public int counter = 0;

	public ItemFillStick() {
		setUnlocalizedName(Reference.ModItems.FILLSTICK.getUnlocalizedName());
		setRegistryName(Reference.ModItems.FILLSTICK.getRegistryName());
	...
	public void BreakBlocks(EntityPlayer player, World worldIn) {
		...
		player.sendMessage(new TextComponentString("Filling Blocks"));
		int sX = 1, sY = 1, sZ = 1;
		...
		for (int i = 0; i <= Math.abs((int) (posA[1].getX()) - (int) (posA[0].getX())); i++) {
			for (int j = 0; j <= Math.abs((int) (posA[1].getY()) - (int) (posA[0].getY())); j++) {
				for (int k = 0; k <= Math.abs((int) (posA[1].getZ()) - (int) (posA[0].getZ())); k++) {
					...
					worldIn.setBlockState(p, Blocks.STONE.getDefaultState(), 3);
