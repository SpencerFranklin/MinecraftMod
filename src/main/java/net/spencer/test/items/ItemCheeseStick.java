...
public class ItemCheeseStick extends Item {
	...
	public ItemCheeseStick() {
		setUnlocalizedName(Reference.ModItems.CHEESESTICK.getUnlocalizedName());
		setRegistryName(Reference.ModItems.CHEESESTICK.getRegistryName());
	...
	public void BreakBlocks(EntityPlayer player, World worldIn){
		...
		player.sendMessage(new TextComponentString("Cheese-ing Blocks"));
		int sX = 1, sY = 1, sZ = 1;
			...
			for (int j = 0; j <= Math.abs((int) (posA[1].getY()) - (int) (posA[0].getY())); j++) {
				for (int k = 0; k <= Math.abs((int) (posA[1].getZ()) - (int) (posA[0].getZ())); k++) {
					...
					if (!b.isAir(worldIn.getBlockState(p), worldIn, p)) {
						worldIn.setBlockState(p, ModBlocks.cheese.getDefaultState(), 3);
